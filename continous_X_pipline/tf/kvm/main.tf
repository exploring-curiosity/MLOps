resource "openstack_networking_network_v2" "private_net" {
  name                  = "private-net-birdclef-${var.suffix}"
  port_security_enabled = false
}

resource "openstack_networking_subnet_v2" "private_subnet" {
  name       = "private-subnet-birdclef-${var.suffix}"
...
resource "openstack_networking_port_v2" "private_net_ports" {
  for_each              = var.nodes
  name                  = "port-${each.key}-birdclef-${var.suffix}"
  network_id            = openstack_networking_network_v2.private_net.id
  port_security_enabled = false
...
resource "openstack_networking_port_v2" "sharednet2_ports" {
  for_each   = var.nodes
    name       = "sharednet2-${each.key}-birdclef-${var.suffix}"
...
resource "openstack_compute_instance_v2" "nodes" {
  ...
  name        = "${each.key}-birdclef-${var.suffix}"
  ...
  user_data = <<-EOF
    #! /bin/bash
    sudo echo "127.0.1.1 ${each.key}-birdclef-${var.suffix}" >> /etc/hosts
